...
namespace  DungeonCrawler.PlayerMonoAssembly
{
    ...
    internal class PlayerController : MonoBehaviour
    {
        ...
        void Move(MovementAction action)
        {
            ...
            if (_mapSwitcher.CurrentDungeon.Map.GetSingleEntity<CharacterWall>(newGridPosition) != null)
            {
                // Debug.Log("Move is cancelled because new position is wall");
                return;
            }
            else if (_mapSwitcher.CurrentDungeon.Map.GetSingleEntity<CharacterStairs>(newGridPosition) != null)
            {
